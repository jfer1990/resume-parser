...
const saveAssignments = async(candidates, reviewerID, reviewerObject)=>{ //esto es parte del controlador? o es un helper? 
    try{
...
const getTodayRevision = async(req,res = response) => {
    ...
    function shuffle(array) {//saca esta función de aquí!!! aumenta complejidad ciclomática
...
//refactorizar-> modularizar y bajar complejidad ciclomática y de comprensión. 
    const assignments = await Promise.all(reviewers.map(async(reviewer)=>{
        ...
        const today = moment().startOf('day').toDate() ; 
        const {candidates : cands} = await Revision.findOne({reviewerID:new mongoose.Types.ObjectId( reviewer.id ) , "date": today}).exec();
